...
/**
 * The nth occurrence of `weekday` (`getDay()` convention, 0 = Sunday) in a
 * given month, or its last occurrence when `n` is `'last'`. `n` in 1-4 is
 * guaranteed to exist for every weekday in every month; a 5th occurrence
 * only sometimes does, which is exactly why this doesn't accept `n: 5` —
 * callers wanting "the last one" (which may or may not be a 5th) ask for
 * `'last'` explicitly instead.
 */
